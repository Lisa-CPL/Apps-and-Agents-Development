    ...
    <div className="min-h-screen pb-24 bg-transparent">
      ...
      <main className="px-5 pt-8 space-y-10">
        <section className="grid grid-cols-2 gap-4">
           <div className="col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-cpl-border flex items-center justify-between relative overflow-hidden group">
              <div className="relative z-10">
                 <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Growth Streak</p>
                 <h2 className="font-serif text-3xl text-cpl-blue">4 Day Streak</h2>
              </div>
              <div className="bg-blue-50 p-3 rounded-full relative z-10 group-hover:scale-110 transition-transform duration-500">
                 <Flame className="w-8 h-8 text-cpl-blue fill-current" />
              </div>
           ...
           <div className="bg-white p-5 rounded-2xl border border-cpl-border shadow-sm">
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Skills Mastered</p>
              <div className="flex items-baseline gap-2">
                 <span className="text-3xl font-serif text-cpl-red font-black">7</span>
           ...
           <div className="bg-white p-5 rounded-2xl border border-cpl-border shadow-sm">
              ...
              <div className="flex items-baseline gap-2">
                 <span className="text-3xl font-serif text-cpl-blue font-black">12</span>
        ...
        <section>
           ...
           <div className="space-y-4">
              {MINI_APPS.slice(0, 3).map((app, i) => (
                <div key={app.id} className="bg-white p-5 rounded-2xl border border-cpl-border hover:shadow-md transition-shadow">
                   <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="flex gap-0.5 text-cpl-amber">
                         {[1,2,3,4,5].map(star => (
                           <Star key={star} className={`w-3 h-3 ${star <= 5-i ? 'fill-current' : 'text-gray-100'}`} />
                         ))}
                      </div>
                   </div>
                   ...
                   <p className="text-right text-[10px] font-bold text-gray-400 mt-2">{85 - i * 15}% Mastered</p>
                </div>
              ))}
           </div>
        ...
        <section>
           ...
           <div className="flex gap-6 overflow-x-auto pb-4 hide-scrollbar -mx-5 px-5">
              {[
                { color: 'bg-amber-400', icon: <Award className="w-10 h-10" />, title: 'First Reflect' },
                { color: 'bg-blue-600', icon: <History className="w-10 h-10" />, title: 'Bias Buster' },
                { color: 'bg-red-500', icon: <TrendingUp className="w-10 h-10" />, title: 'Empathy Pro' },
                { color: 'bg-green-500', icon: <Award className="w-10 h-10" />, title: 'Lab Champ' }
              ].map((badge, i) => (
        ...
        <section className="pb-12">
           <div className="flex justify-between items-center mb-6">
              ...
              <button className="text-xs font-bold text-cpl-blue">History</button>
           </div>
           <div className="bg-white rounded-2xl border border-cpl-border overflow-hidden">
              {[
                { title: 'Critical Bias Lab', date: 'Oct 24', dur: '12 mins', score: '92%' },
                { title: 'Conflict Resolution', date: 'Oct 22', dur: '20 mins', score: '78%' },
                { title: 'Insight Check', date: 'Oct 21', dur: '8 mins', score: '100%' }
              ].map((item, i) => (
                  ...
                  <div className="p-4 flex justify-between items-center hover:bg-gray-50 transition-colors cursor-pointer group">
                     <div className="flex items-center gap-4">
                        ...
                        <div>
                           ...
                           <p className="text-[10px] text-gray-400 font-medium">{item.date} • {item.dur}</p>
                        </div>
                     </div>
                     <span className={`text-lg font-serif font-black ${parseInt(item.score) >= 90 ? 'text-cpl-blue' : 'text-cpl-red'}`}>
                        {item.score}
                     </span>
                  </div>
                </React.Fragment>
