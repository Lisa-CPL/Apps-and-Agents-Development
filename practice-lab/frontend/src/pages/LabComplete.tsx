...
import { FeedbackPayload, fetchMiniAppDetail } from '../api';
...
export const LabComplete: React.FC = () => {
  ...
  const state = location.state as { feedback?: FeedbackPayload } | null;
  const feedback = state?.feedback ?? null;
  const score = feedback ? computeScore(feedback) : 0;

  const [appName, setAppName] = useState<string>('');
  const [estimatedTime, setEstimatedTime] = useState<string>('');

  useEffect(() => {
    if (!appId || hasRecorded.current) return;
    fetchMiniAppDetail(appId).then((d) => {
      setAppName(d.name);
      setEstimatedTime(d.estimated_time);
      completeLab(appId, score, d.estimated_time);
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col relative overflow-hidden">
      ...
      >
        ...
        <motion.div variants={itemVariants} className="text-center mb-12">
          ...
          <p className="text-gray-400 font-medium leading-relaxed max-w-[280px] mx-auto">
            ...
            <strong>{appName || '—'}</strong> practice module.
          </p>
        </motion.div>
        ...
        <div className="w-full space-y-4 mb-20 max-w-md">
          <motion.div variants={itemVariants} className="bg-white/90 backdrop-blur-sm p-6 rounded-3xl flex justify-between items-center border border-cpl-border shadow-sm">
            ...
            <span className="text-cpl-blue font-black font-serif text-xl">{score}/100</span>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="bg-white/90 backdrop-blur-sm p-6 rounded-3xl flex justify-between items-center border border-cpl-border shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="font-serif text-lg text-cpl-dark">{estimatedTime || '—'}</p>
              </div>
            </div>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-4xl p-8 bg-white transition-all duration-300 z-20">
        <button
